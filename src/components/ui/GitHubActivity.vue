...
import { tryit, sort } from 'radashi'
...
<template>
  <div class="space-y-1.5">
    ...
    <div v-if="loading" class="flex gap-3">
      <div v-for="i in 3" :key="i" class="h-3 bg-muted rounded animate-pulse" :style="{ width: `${60 + i * 10}px` }" />
    </div>
    ...
    <!-- Repos — inline compact -->
    <div v-else class="flex flex-wrap gap-x-4 gap-y-1">
      <a v-for="repo in recentRepos" :key="repo.id" :href="`https://github.com/${repo.repo}`" target="_blank"
        rel="noopener noreferrer"
        class="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors">
        <span>📦</span>
        <span class="font-medium">{{ repo.name }}</span>
        <span v-if="repo.stars > 0" class="text-muted-foreground/50">⭐{{ repo.stars }}</span>
        <span class="text-muted-foreground/40">· {{ getRelativeTime(repo.pushedAt) }}</span>
      </a>
    </div>
  </div>
</template>
